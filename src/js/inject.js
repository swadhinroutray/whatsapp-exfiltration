...
chat = async () => {
  let resolveChat = new Promise(function (resolve, reject) {
    chrome.storage.sync.get({ chat: true }, function (options) {
      resolve(options.chat);
    });
  });
  //enter chat
...
inject = async () => {
  let resolveKill = new Promise(function (resolve, reject) {
    chrome.storage.sync.get({ kill: false }, function (options) {
      resolve(options.kill);
    });
  });
  ...
  let resolveChat = new Promise(function (resolve) {
    chrome.storage.sync.get({ chatFinal: true }, function (options) {
      resolve(options.chatFinal);
    });
  });
  ...
  let resolveValues = new Promise(function (resolve) {
    chrome.storage.sync.get({ values: true }, function (options) {
      resolve(options.values);
    });
  });
  let resolveResponse = new Promise(function (resolve) {
    chrome.storage.sync.get({ response: true }, function (options) {
      resolve(options.response);
    });
  });
  let values = await resolveValues;
  ...
  let monitorChats = async function () {
    return (async function () {
      ...
      count = len === 0 ? 0 : len - 1;
      if (len != iniLen) {
        incoming = document.querySelectorAll(
          ".selectable-text.copyable-text span"
        )[count].textContent;
            ...
            if (!checkInput(incoming, item.values.split(","))) {
              ...
              for (i = 0; i < responses.length; i++) {
                ...
                if (responses[0].trim() && values.trim()) {
                  document
                    .querySelector('[data-testid="compose-btn-send"]')
                    .click();
                }
              }
            }
  ...
  startTimer2 = async () => {
    setInterval(monitorChats.bind(), 200);
  };
  startTimer2();
};
startTimer();
